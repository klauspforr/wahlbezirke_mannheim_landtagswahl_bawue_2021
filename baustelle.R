...
ding<-streetdata[streetdata[,4]=="F",]
q<- opq(bbox = 'Mannheim') %>%
  #add_osm_feature(key = 'addr:housenumber', value = '3', value_exact = F) %>%
  add_osm_feature(key = 'addr:place', value = 'B1') %>%
  add_osm_feature(key = 'addr:city', value = 'Mannheim') %>%
  osmdata_sf ()
streetdata2<-rbind(streetdata2,streetdata[1,c(1,2,3,7)])
streetdata2<-rbind(streetdata2,cbind(as.matrix(cbind(q$osm_polygons$addr.place,
                                                     q$osm_multipolygons$addr.place,
                                                     q$osm_polygons$addr.street,
                                                     q$osm_polygons$addr.street)),
                                     as.matrix(cbind(q$osm_polygons$addr.housenumber,
                                                     q$osm_multipolygons$addr.addr.housenumber)),"",1111))
plot(q$osm_polygons)
q$osm_multipolygons$addr.housenumber
q$osm_polygons$addr.housenumber
geocode_OSM("Mannheim, A1,2")$coords
ggmap(get_map(getbb("Mannheim"),source="osm"))+geom_sf(aes(alpha=0),data=st_as_sf(Wahlbezirke),inherit.aes=F)

available_features()
nrow(q$osm_points)
nrow(q$osm_polygons)
nrow(q$osm_multipolygons)
ding<-rbind(ding,streetdata[1,])
plot(q$osm_polygons)
...
ggmap(get_map(getbb("Mannheim, Neckarstadt"),source="osm"))
x <- clickpoly(add = TRUE) # Start locator overlayed on region
plot(x)